...
type Category = "Agents" | "W-Engines" | "Drive Discs" | "Bangboos";
...
function Search() {
    ...
    const data: Data[] = [
        ...characters.map((char) => ({
            ...
            category: "Agents" as Category,
        })),
        ...weapons.map((wep) => ({
            name: wep.name,
            displayName: wep.displayName,
            category: "W-Engines" as Category,
        })),
        ...driveDiscs.map((disc) => ({
            name: disc.name,
            displayName: disc.displayName,
            category: "Drive Discs" as Category,
        })),
        ...bangboos.map((bangboo) => ({
            name: bangboo.name,
            displayName: bangboo.displayName,
            category: "Bangboos" as Category,
        })),
    ];
...
function getImageIcon(category: Category, name: string) {
    switch (category) {
        case "Agents":
            return `characters/icons/${name}`;
        case "W-Engines":
            return `w-engines/${name}`;
        case "Drive Discs":
            return `drive-discs/${name.split(" ").join("_")}`;
        case "Bangboos":
            return `bangboos/${name}`;
    }
}

function getURL(option: Data) {
    return `${option.category.toLowerCase().split(" ").join("-")}/${option.name
        .toLowerCase()
        .split(" ")
        .join("_")}`;
}
